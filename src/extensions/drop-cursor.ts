...
class DropCursorView {
  ...
  updateOverlay() {
    ...
    const isBlock = !$pos.parent.inlineContent; let rect;
    if (isBlock) {
      ...
      if (before || after) {
        ...
        if (node) {
          ...
          if (before && after) { top = (top + (this.editorView.nodeDOM(this.cursorPos!) as HTMLElement).getBoundingClientRect().top) / 2; };
    ...
    if (!this.element) {
      ...
      this.element.style.cssText = 'position: absolute; z-index:11; pointer-events: none;';
      if (this.color) {
  ...
  dragleave(event: DragEvent) {
    if (event.target === this.editorView.dom || !this.editorView.dom.contains((event as any).relatedTarget)) { this.setCursor(null); };
  }
}
...
export default Extension.create<DropcursorOptions>({
  name: 'dropCursor',
