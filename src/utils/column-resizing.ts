...
import { TableMap, tableNodeTypes, pointsAtCell, cellAround } from 'prosemirror-tables';
import { TableView, updateColumns } from '@/components/ExtensionViews/tableView';
...
function handleMouseMove(
  ...
  if (!pluginState.dragging) {
    ...
    // let align = 'default';
    // let lastColumnResizableValue = lastColumnResizable;
    // if (target?.tagName === 'TH') {
    //   const tableElement = target!.closest('table');
    //   align = tableElement!.align;
    //   if (align === 'right') {
    //     lastColumnResizableValue = !lastColumnResizable;
    //   }
    // }
    let cell = -1;
    if (target) {
...
export function handleDecorations(
  ...
  // Добавляем стили ресайза для первой черточки таблицы
  if (col === 0) {
    const pos = start;
